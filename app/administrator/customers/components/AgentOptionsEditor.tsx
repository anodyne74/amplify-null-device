...
/** The "agent card" — a chip-based add/remove list of agent initials for a customer.
 * Shared by the Create Customer form and the Edit Customer panel so both present the
 * same UI. These codes are what show up on the operator's run sheet when generating
 * routes for this customer, so only initials are needed — not full agent names. */
    ...
    <div className={styles.fieldsGridFull}>
      ...
      <div className={styles.agentChipRow}>
        {agentOptions.map((option) => (
          <Tag key={option} onRemove={disabled ? undefined : () => onRemove(option)}>
            {option}
          </Tag>
        ))}
      </div>
